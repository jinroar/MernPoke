  ...
  const enemy = async (pokemon: typeof pokemon2Data) => {
    ...
    if (pkmn2Hp <= 0) {
      ...
      addItem(
        `${
          pokemon2Data?.name
        } " used " ${move?.name.toUpperCase()}  with a power of  ${
          move?.power || 0
        }`
      );
  ...
  return (
    <div
      className="  bg-no-repeat bg-cover overflow-hidden  bg-[url('https://i.pinimg.com/originals/08/d9/ef/08d9ef7723de602edefa8af825d9a1e2.gif')]   
          ...
          text-poke-yellow"
    >
      <button className="reltive inset-y-0  w-70 h-10  text-center  border-poke-yellow border-spacing-2 border-4">
        <Link to="/About">Back to Menu</Link>{" "}
      </button>

      <div className=" grid-rows-2 border-tahiti border-spacing-2 border-4">
        <div className="grid grid-rows-2 grid-flow-col gap-4 border-purple border-spacing-2 border-4">
          <div className="w-[500px] h-[500px] overflow-auto border-poke-yellow border-spacing-2 border-4">
            01
            <button
            ...
            {capture && (
              <>
                ...
                <strong className="fight">
                  {" "}
                  🔥 Capture Rate ??????? {100 - 50 * (pkmn1Hp / defaultHp1)}%
                  🔥
                </strong>{" "}
                <br></br>
                {showHideButton ? (
                ...
                <img
                  ...
                  src="https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTE0OWRheXo4dDVqOXgydDdnNDRiZHBlY3VpMGZjbTN3Z2tqcGgxbiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/7OzAnGdw9bWTH5iroJ/giphy.gif"
                />
              </>
            )}
            {stored && (
              <>
                ...
                <div>
                  <button className=" h-20 w-70 top-40    hover:border-poke-yellow border-spacing-2 border-4 ...">
            ...
            {pokemon2Data && (
              <div className="border-poke-yellow border-spacing-2 border-4 grid place-content-start md:place-content-start ">
                ...
                <p>
                  ...
                  HP:{" "}
                  {
                    pokemon2Data.stats.find((e) => e.stat.name === "hp")
                      ?.base_stat
                  }{" "}
                  <br />
                  Attack:{" "}
                  {
                    pokemon2Data.stats.find((e) => e.stat.name === "attack")
                      ?.base_stat
                  }{" "}
                  <br />
                  Defense:{" "}
                  {
                    pokemon2Data.stats.find((e) => e.stat.name === "defense")
                      ?.base_stat
                  }{" "}
                  <br />
                  <br />
                  <AudioPlayer
                    src={pokemon2Data.cries.latest}
                    controls
                    volume={0.5}
          ...
          <div className="grid grid-col-subgrid gap-4 col-span-3">
            <div className="row-start-2">
              05
              {pokemon1Data && (
                <div className=" right-20 border-poke-yellow border-spacing-2 border-4  grid place-content-start md:place-content-end ">
                  <img
                    className="size-44"
                    src={
          ...
          <div className="grid grid-col-subgrid gap-4 col-span-3">
            <div className="row-start-2">
              ...
              {move1 && (
                <div
                  className="bg-transparent p-8 rounded-lg
                        border-2 border-white shadow-lg  
                        "
                  onClick={() => payt(move1, 1)}
                >
                  <h1 className="text-3xl font-bold mb-4">{move1.name}</h1>
                  <p className="text-lg">Damage: {move1.power || memapower}</p>
                </div>
              )}
              {move2 && (
                <div
                ...
                >
                  ...
                  <p className="text-lg">Damage: {move2.power || memapower}</p>
                </div>
              )}
            </div>
            <div className="row-start-2">
        ...
        <div className="fight">
          {hasFightResults && (
            <>
              <strong className="fight">
                {" "}
                🔥 {pokemon1Data?.name.toUpperCase()}: {pkmn1Hp} 🔥
              </strong>{" "}
              <br></br>
              <strong className="fight"> 🔥 vs 🔥</strong> <br></br>
              <strong className="fight">
                {" "}
                🔥 {pokemon2Data?.name.toUpperCase()}: {pkmn2Hp} 🔥
              </strong>{" "}
              <br></br>
            </>
          )}
