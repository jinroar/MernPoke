...
const PickPokemon = () => {
  ...
  const selected = async () => {
    for (let i = 1; i < 3; i++) {
      ...
      const fetchJson2 = await fetch(
        `https://pokeapi.co/api/v2/pokemon/${num2}`
      );
      const pokemonData2 = await fetchJson2.json();
      setPOkemon2Data(pokemonData2);

      const fetchJson3 = await fetch(
        `https://pokeapi.co/api/v2/pokemon/${num3}`
      );
  ...
  const fight = () => {
    const pkmn1Hp =
      ...
      (pokemon1Data?.stats.find((s) => s.stat.name === "defense")?.base_stat ||
        0) -
      (pokemon2Data?.stats.find((s) => s.stat.name === "attack")?.base_stat ||
        0);
    ...
    const pkmn2Hp =
      ...
      (pokemon2Data?.stats.find((s) => s.stat.name === "defense")?.base_stat ||
        0) -
      (pokemon1Data?.stats.find((s) => s.stat.name === "attack")?.base_stat ||
        0);
    setPkmn2Hp(pkmn2Hp);
  ...
  return (
    <div
      className=" overflow-hidden bg-no-repeat bg-cover bg-[url('https://wallpaperaccess.com/full/8406757.gif')] max-w-screen w-screen h-screen max-h-screen  grid-cols-4 content-center ...
        text-poke-yellow"
    >
      ...
      <div className="relative inset-y-2 left-52  my-8 h-56 grid grid-cols-4 row-auto gap-4 content-center ...">
        ...
        {pokemon2Data && (
          <div
            className="border-poke-yellow border-spacing-2 border-4 bg-grey-glass bg-opacity-20 hover:bg-tahiti/25 content-center"
            onClick={() => selectPokemon(pokemon2Data?.name)}
          >
        ...
        {pokemon3Data && (
          <div
            className="border-poke-yellow border-spacing-2 border-4 bg-grey-glass bg-opacity-20 hover:bg-tahiti/25 content-center"
            onClick={() => selectPokemon(pokemon3Data.name)}
          >
            <Link to="/SearchPokemon" state={pokemon3Data.name}>
              <div className="relative group conte">
                ...
                <img
                  src={
                    pokemon3Data.sprites.other.showdown.front_default ||
                    defaultGif
                  }
                  alt="Beautiful Landscape"
                  className=" centeerw-[100px] h-[120px] transition-opacity duration-300 ease-in-out opacity-100 group-hover:opacity-0"
                ...
                <img
                  src={
                    pokemon3Data.sprites.other.showdown.back_default ||
                    defaultGif
                  }
                  alt="Serene Nature"
                  className="w-[200px] h-[120px] transition-opacity duration-300 ease-in-out opacity-0 group-hover:opacity-100"
                ...
                <p>
                  ...
                  HP:{" "}
                  {
                    pokemon3Data.stats.find((e) => e.stat.name === "hp")
                      ?.base_stat
                  }{" "}
                  <br />
                  Attack:{" "}
                  {
                    pokemon3Data.stats.find((e) => e.stat.name === "attack")
                      ?.base_stat
                  }{" "}
                  <br />
                  Defense:{" "}
                  {
                    pokemon3Data.stats.find((e) => e.stat.name === "defense")
                      ?.base_stat
                  }{" "}
                  <br />
                  <br />
                  <AudioPlayer
                    src={pokemon3Data.cries.latest}
                    controls
                    volume={0.5}
                    onPlay={() => console.log("Playing")}
                    onPause={() => console.log("Paused")}
      ...
      <div className="box5">
        ...
        <div className="invisible">
          Press the fight button again to refresh.
        </div>
      </div>
